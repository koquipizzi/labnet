...
class InformeController extends Controller
{
	...
	public function actionEntregar($accion, $estudio = null, $id)
	{
		...
		if ($model) {
			...
			\Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
			return ['rta' => 'error', 'message' => 'Fallo'];
		}
	...
	public function actionPublicar($id, $estudio)
	{
		$response = ['rta' => 'error', 'message' => 'No se pudo subir correctamente el informe'];
		try {
			// throw new ErrorException('Este si lo muestra');
			...
			switch ($estudio) {
				case \app\models\Estudio::getEstudioPap(): //pap
					...
					$vista = '_print_pap';
					break;
				case \app\models\Estudio::getEstudioBiopsia():
					$vista = '_print_biopsia';
					break;
				case \app\models\Estudio::getEstudioCitologia():
					$vista = '_print_inf_cito';
					break;
				case \app\models\Estudio::getEstudioInmuno():
					$vista = '_print_inf';
					break;
				case \app\models\Estudio::getEstudioMolecular():
					$vista = '_print_mole';
					break;
			}
			...
			if ($model) {
				$modelp = $model->protocolo;
				$dni = $modelp->getPacienteDoc();
			}

			$mainFolder = Yii::getAlias('@webroot') . "/uploads/pdf/estudio";
			$nombre_archivo_pdf = "$mainFolder/$dni/$dni" . "_" . "$id.pdf";
			...
			$this->checkLocalDir($mainFolder);
			$this->checkLocalDir("$mainFolder/$dni");

			$pdf->render();

			$this->uploadInforme($nombre_archivo_pdf, $dni);

			$response = ['rta' => 'ok', 'message' => 'El informe se subio correctamente.'];
		} catch (\Throwable $th) {
			Yii::$app->response->statusCode = 404;
			$response = ['rta' => 'error', 'message' => $th->getMessage()];
		}
	...
	/**
	 * Funcion que recibe path de archivo local a subir
	 */

	function uploadInforme($nombre_archivo_pdf, $dni)
	{
		if (file_exists($nombre_archivo_pdf)) {
			try {
				$this->checkRemoteDir(self::REMOTE_DIR);
				$this->checkRemoteDir(self::REMOTE_DIR . '/' . $dni);

				$ftpserver = Yii::$app->ftpFs;

				$exists = $ftpserver->has(self::REMOTE_DIR . '/' . $dni . '/' . basename($nombre_archivo_pdf));

				//Si existe informe, lo elimina.
				if ($exists) {
					$result = $ftpserver->delete(self::REMOTE_DIR . '/' . $dni . '/' . basename($nombre_archivo_pdf));
				}

				//Crea nuevo informe.
				$stream = fopen($nombre_archivo_pdf, 'r+');

				$result = $ftpserver->writeStream(
					self::REMOTE_DIR . '/' .  $dni . '/' . basename($nombre_archivo_pdf),
					$stream
				);
				fclose($stream);
			} catch (\Throwable $th) {
				// throw new \Exception("Error no se pudo conectar con el servidor, verifique su conexion.");
				throw new \Exception($th->getMessage());
			}

			if (!$result) {
				throw new \Exception("Error no se pudo publicar el informe corretamente.");
			}
		} else {
			throw new \Exception("Error no se encontró el archivo a publicar.");
		}
	}

	function checkRemoteDir($path)
	{
		$ftpserver = Yii::$app->ftpFs;
		if (!$ftpserver->has($path)) {
			$ftpserver->createDir($path);
		}
	}

	/**
	 *	Funcion recibe un path y verifica si existe, de lo contrario crea el directorio
	 */

	function checkLocalDir($path)
	{
		if (!file_exists($path)) {
